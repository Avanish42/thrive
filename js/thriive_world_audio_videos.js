...
//            if (user) {
                document.getElementById(content).setAttribute('src', snapshot.child(content).val()+"controls='true' autoplay muted loop");
//            } else {
//                myFunction(); 
//            }
...
function myFunction() {
    firebase.auth().onAuthStateChanged(user => {
            if(user) {
                ...
                document.getElementById('url').pause();
                document.getElementById('downloadicon').style.display="none";
            }
        });
}

function addtomydashboard(){    
    //Check if User Already Exist
 var ref = firebase.database().ref();
 var userref = ref.child("users");
 var user = firebase.auth().currentUser;
 if (user) {
     ...
     vidref.child(videoId).once('value', function (videoshot) {
         var videodetails = videoshot.val();
         var url = videoshot.child('url').val();
         userref.orderByChild("id").once('value', function (snapshot) {
             snapshot.forEach(function (childuser) {
                 if (email == childuser.child("email").val()) {
                     var id = childuser.key;
                     var videoref = userref.child(id);
                     videoref.child('video').once('value', function (checksnap) {
                         var count = 0;
                         checksnap.forEach(function (videochild) {
                             if (url == videochild.child('url').val()) {
                                 console.log(url);
                                 console.log(videochild.child('url').val());
                                 count = count + 1;
                             }
                         });
                         if (count > 0) {
                             alert("video has already in your dashboard");
                         }
                         else {
                             videoref.child("video").push(videodetails);
                         }
                     });
                 }
             });
